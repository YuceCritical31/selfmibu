...
if(!Atahan) return message.channel.send(new Discord.MessageEmbed().setDescription(`${basarisiz} ${message.author}, İsmini belirtmelisin.`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
...
message.member.cache.get(ayarlar.sahip).editUsername(Atahan) 
return message.channel.send(new Discord.MessageEmbed().setDescription(`${basari} ${message.author}, Başarılı bir şekilde ismin \`${Atahan}\` olarak değiştirildi.`).setFooter(`NOT: 3 Kezden Fazla Kullanınca Değişmeyebilir Biraz Sonra Tekrar Deneyin Olucaktır.`, message.guild.iconURL({dynamic: true})).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('0x348f36').setTimestamp())
.catch(error => {
