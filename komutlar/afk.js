...
message.channel.send(new MessageEmbed().setDescription(`${basarili} ${message.author}, Başarıyla \`${sebep} ${sebep2}\` sebebiyle afk oldunuz.`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('0x348f36').setTimestamp()).then(x => x.delete({timeout: 5000}));
db.set(`afk`, "Açık")
